...
- Validation: 22 Creaa Python tests + 19 Node extension tests pass; full Python suite 128 pass / 1 pre-existing `test_api_captcha_fingerprint` failure. PR #14 merged as `5e63d48`; Coolify deployment `rfusrjcvqwgvswwyxzrhweve` finished 2026-09-07 19:36 UTC. Live `/health`, `/v1/creaa/accounts`, `/v1/creaa/models` return 200. No live Creaa generation has succeeded yet: browser registration is pending (0 accounts/models). Existing deployed Flow paths unchanged. Creaa extension is opt-in; models come from the connected website account.
- Owner manually loaded extension 3.4.0 during build. Automated extension management AND extension settings were blocked by browser URL policy. Owner must do final reload and enable Creaa in the popup before end-to-end testing. Settings default to existing Flow backend/key. API documentation: `docs/creaa-api.md`.

- Production runs `5e63d48` (PR #14), verified deployment `rfusrjcvqwgvswwyxzrhweve` finished healthy, extension package **3.3.10**. `feat/async-submit-status` remains unmerged; leave it alone.

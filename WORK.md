...
- **2026-09-04 12:04-12:20 UTC, owner's browser on 3.3.9: mints succeed (65x 200 in 15 min) but the Labs tab still reloads/reopens and the Chrome window sometimes vanishes.** Root causes CONFIRMED from the pasted extension log + code, fixed in **3.3.10** (PR #10, `worker-extension/background.js`):
  ...
  - SHIPPED 2026-09-04 12:25 UTC: PR #10 merged as `5247d6c`, container redeployed, `worker-latest.zip` = 3.3.10 (backups 3.3.7/3.3.8/3.3.9 kept beside it). Owner's browser reported `ext_version=3.3.10` at 12:24:52 and the Labs tab now opens in the owner's existing window (screenshot 12:23 UTC). Live behaviour under load still UNVERIFIED as of 12:27 — what settles it: no `Replaced the Labs tab` / `Left tab open` lines in the owner's popup log over ~10 min of minting, and no `No current window` on the server. Rollback: copy `worker-3.3.9.backup.zip` over `worker-latest.zip`.

...
- **Still broken after 3.3.7 — the mint itself now fails.** Once the owner's browser reloaded to 3.3.7 the `did not reach Flow URL` error stopped for good (tab loop fixed), but requests then stall at `status_code=102 / solving_image_captcha / progress=38` and the extension reports `worker failed: empty token result` plus `Flow error reported (ignoring): reCAPTCHA error`. So the failure moved one stage later: the tab now reaches Flow, but `grecaptcha.enterprise.execute()` yields no token. CONFIRMED 2026-09-03 21:38-21:41 UTC.
- Ruled out — **the reCAPTCHA site key did NOT change with the domain move.** `worker-extension/background.js:23` hard-codes `6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV`; a live fetch of `https://flow.google.com/` (HTTP 200, 147 KB) serves that same key. Do not spend time on a key-mismatch theory. CONFIRMED.
- **Diagnostic defect blocking root-cause: the real reCAPTCHA error is discarded.** In `mintTokenInTab`, the injected MAIN-world function *rejects* on failure, but a rejected promise does not survive the `chrome.scripting.executeScript` boundary — Chrome returns `result: undefined` and the reason is lost, so every distinct failure (refused execution, blocked `enterprise.js`, timeout, empty token) surfaces identically as `empty token result`. Google's actual verdict has never reached the server log. CONFIRMED by reading the code.
- **Token 55 is healthy and this is not quota.** 207 successes today, last at 08:51:44 UTC; `is_active=1`, no ban; all three rows in `model_quota_cooldowns` expired in August. Its recorded errors today stop at hour 11 because everything after that dies at the captcha stage and never records a response body.
- **Probably TWO stacked problems, not one.** Before the domain move this account was already hitting `PUBLIC_ERROR_UNUSUAL_ACTIVITY_TOO_MUCH_TRAFFIC: reCAPTCHA...` (11 at hour 08, 16 at hour 09, 1 at hour 11) — the known per-IP reCAPTCHA reputation problem where one home IP mints cleanly for only one account. That is unrelated to the domain move and was not fixed by anything on 2026-09-03. NOTE the stages differ: UNUSUAL_ACTIVITY happens *after* a token is minted (generate call rejected), whereas tonight's `empty token result` happens *before* (no token at all). Do not assume they are the same cause without evidence.
- The fleet-wide zero successes in hours 20-21 UTC is mostly NOT reCAPTCHA: of 10,893 requests since 20:00, **10,754 are 503** (fast-fail, no eligible account — staff asleep, others out of daily quota). Only 113 reached a browser at all.
...
- **Branch `fix/surface-recaptcha-mint-error` (commit `d1d4ed1`, ext 3.3.8) is checked out in the Mac working tree on purpose.** It makes the injected mint function resolve a verdict object `{ok, token|err, url, hadGrecaptcha}` instead of rejecting, so the server log will show the real reason plus page context (e.g. `recaptcha execute failed [url=..., grecaptcha=present]`). Diagnostic only — it does not change when or how often a mint is attempted. `node --check` passes on both JS files. NOT merged, NOT pushed, NOT published to the staff download.
- **Do not `git checkout main` on that Mac while the owner is testing.** Chrome loads the extension unpacked from `worker-extension/`, so switching branches silently reverts their browser to 3.3.7 on the next reload.
- Root cause of `empty token result` is still UNKNOWN as of 2026-09-03 21:49 UTC. What would settle it: reload the owner's browser onto 3.3.8 and read the next `worker failed:` line in the container log.
